...
function VerificationCodeContent() {
  ...
  useEffect(() => {
    if (!email && typeof window !== 'undefined') {
      console.warn(
        'No recruiter email found. User should be redirected to signup.'
      )
    }
  }, [email])
  ...
  const handleVerifyOTPCode = useCallback(async () => {
    ...
    try {
      const response = await RecruiterAuth.validateRegistration({
        token: enteredCode,
        email: email,
      })
      ...
      if (response.status === 200) {
        ...
        setTimeout(() => {
          router.push('/welcome-onboarding?type=recruiter')
        }, 5000)
      }
    } catch (error) {
  ...
  const handleResendOTP = async (): Promise<void> => {
    ...
    setIsLoading(true)
    try {
      const response = await RecruiterAuth.resendOTP(email)
  ...
  const CheckIcon: IconType = FaCheck
  const TimesIcon: IconType = FaTimes
